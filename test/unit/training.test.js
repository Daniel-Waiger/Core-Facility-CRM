...
describe('#47 demo seed', () => {
  test('seedSampleData() loads 17 training rows covering valid, expired, not-yet-valid and NULL-start, and booking #1 still totals 589.95', async () => {
    const { DB, UI } = await freshDb({ search: '?demo=1' });
    const ok = await DB.seedSampleData({ force: true });
    assert.equal(ok, true);

    assert.equal(DB.row('SELECT COUNT(*) c FROM person_instrument_training').c, 17, 'the demo dataset must seed exactly 17 training rows');
