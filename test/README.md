...
| `training.test.js` | Issue #47 (Person Profile + Instrument Training): the migration (`people.mobile`/`campus`, `person_instrument_training`), the ref-counters (a training row counts for the trainee/instrument, never for a bare `trainer_id`), `retirePerson`/`retireInstrument` retiring rather than deleting a trained person/instrument (and `trainer_id` nulling on a real delete), the `ON DELETE CASCADE` this table relies on, `clearAllData()`, `DB.trainingActiveOn`'s date-boundary rule, `Reports.computeStewardshipRows`'s Trained Users column (distinct people, evaluated as of the range end), `Exports.exportActivityCertificate` and the Stewardship sheet/Notes wording, the Person Profile/Instruments/People screens, `removeTraining`, the demo seed, a source lint of the dispatcher wiring, and the 1.13 rename of the entry level from 'User' to 'Regular' (the fenced migrate() UPDATE extracted from db.js, its idempotency, and that it fires on a backup restore) |
| `instrument-profile.test.js` | The Instrument Profile: em-dash fallbacks, the Xh Ym duration string, the Assisted column, cancelled rows kept and marked, the note preview (tags stripped, clipped, no DOMParser needed), the 14-day upcoming window, and utilization tiles equal to `Reports.computeInstrumentRows` over the whole calendar month (the Reports This Month range), with the Distinct Users tile ignoring a cancelled booking's attendee |
...
| `labels.test.js` | Display helpers that must never alter stored values — money, billing units, milestone statuses, retired names, escaping, the link guard, the shared cancelled-status label and 2-dp rounding |
| `schema.test.js` | Database invariants: the `foreign_keys` pragma `db.export()` silently clears, every cascade, the `projects.pi_id` gap cascade cannot cover, retire/archive, cancellation billing, the denormalized attendee string staying in step with its join table, and the migrated table/index name set matches a fresh schema |
...
| `wiring.test.js` | Every `data-act` has a handler and every handler an emitter; every icon name resolves; no dead icon or CSS class; no file reintroduces the UTC date bug; every file parses |
...
| `exports.test.js` | XLSX/DOCX/PDF export generation: the facility-wide XLSX carrying the same "(Retired)" suffix as the per-project workbook, DOCX/PDF money agreeing with XLSX's waived-cancellation zeroing, the facility-wide Notes sheet appended last, the facility-wide sheet inventory and the DOCX/PDF shared prose builders, the PDF bidi-reversal/font-fallback handling, and the Research Outputs sheet's authors/DOI/URL/acknowledgement/attached-file columns |
