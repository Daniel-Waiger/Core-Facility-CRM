...
| `instrument-profile.test.js` | The Instrument Profile: em-dash fallbacks, the Xh Ym duration string, the Assisted column, cancelled rows kept and marked, the 14-day upcoming window, and utilization tiles equal to `Reports.computeInstrumentRows` for the same range |
