...
**Update (2026-09-19):** 4.3's training records shipped in 1.12.0 (see the clause on the item).
Three items were added as **the next batch**, each in both tracks — a local version in Tier 4
(4.4–4.6) and a server version in Tier 5 (5.8 extended, 5.15, 5.16): an instrument maintenance
and service log, per-lab billing statements for a period, and an iCalendar (.ics) export.
Intended build order for the local batch is 4.5 → 4.4 (with 4.1/4.2 as its data) → 4.6.
...
| 4.3 | ✅ 1.12.0 — **Training/qualification records** — person × instrument with trainer, date, expiry. No auth exists locally, so this is **staff-facing signals, not enforcement** — which matches the early-adopter reality that core staff operate the app: booking-form warnings ("X's training on Y expired"), plus a **"training status unknown" flag** when a person with no training record on an instrument is added to a booking — covering the "did another staff member already train them?" gap, since all staff records live in the same facility database. | M | Feeds 3.2 (trained-user pool trend per instrument) and 3.4. Shipped as records (`person_instrument_training`), the person and instrument profiles, the Trained Users count and scorecard column, and the Activity Certificate export. **Not shipped:** the booking-form expiry warning and the "training status unknown" flag — the booking dialogs do not read the training table yet. |
| 4.4 | **Next — Instrument maintenance & service log** — the record behind 4.1/4.2 made first-class: per instrument, a dated entry with `type` (preventive maintenance / repair / calibration / vendor visit / other), vendor, cost, downtime hours, notes and an optional attachment (scanned service report, photo). Shown as a card on the Instrument Profile page (1.13.0) with the last service date and the total downtime for the year; gives the Stewardship scorecard (3.2) its missing **downtime share** column, so utilization is read against available hours rather than the whole calendar, and gives the activity mix (3.4) its **maintenance** slice. Where a 4.1 downtime record exists for the same instrument and dates, the log entry links to it; a log entry alone never blocks bookings — that stays 4.1's job. | M | New table in `js/db.js` (cascade on instrument delete, explicit delete in `retireInstrument`'s zero-reference branch, added to `clearAllData` and `countInstrumentRefs`); attachments reuse the `files` pattern; downtime hours feed `js/reports.js` so the scorecard and its export read one number. Appears in the Instruments sheet of the facility-wide export and as its own Reports sheet. Absorbs the file-attachment half of 4.2; 4.2 stays as the closed-downtime → log-entry link. |
| 4.5 | **Next — Per-lab billing statements for a period** — the document the "Invoiced" and "Paid" project statuses imply but nothing produces. Pick a lab/group (or a PI, or one project) and a date range; the screen lists every booking and standalone service entry in it with the **Project Costs money rule** (a cancelled booking counts only if `billing_retained`), per-row Subtotal → Before Tax → Total as on the Bookings & Costs sheet, the grant (by name or number per the Settings toggle), and a statement total. Export one DOCX or PDF per lab, or one XLSX for the period covering all labs. **Mark Issued** stores a `statements` row (lab, range, total, issued date, row count) and stamps the included rows, so a later rate change, edit or cancellation on an issued row is flagged on the next statement rather than silently altering history — this is the local answer to "was this already billed?". Marking a statement issued is admin-gated like the other billing decisions. | M | Aggregation in `js/reports.js` so the statement and the Reports screen cannot disagree; exports in `js/exports.js`, one per format. Rows already carry frozen cost snapshots, so no repricing is involved. Feeds the server's 5.10 (SAP export) and 5.16 (server statements) with a proven row layout. |
| 4.6 | **Next — iCalendar (.ics) export** — a downloadable calendar file for a person (their bookings and milestone due dates), an instrument (its bookings), or the whole facility, over a date range. Pure client-side text generation, the same class of hand-off as Email Attendees' `mailto:`; imports into Outlook, Google Calendar and Apple Calendar. Cancelled bookings are written with `STATUS:CANCELLED` and a stable `UID` per booking, so re-importing a newer file updates and cancels rather than duplicating; times are written as floating local times (no `Z`), matching the app's local-calendar-day rule; a booking without a time becomes an all-day event. Optionally dropped beside the silent auto-backup (3.7) so the file is always fresh. **Not a live subscription** — that needs a server (5.8). | S | Generator in a small new `js/ical.js` (load order after `ui.js`), buttons on the person and instrument profiles and on the Calendar screen. Test the date handling under `TZ='Asia/Jerusalem'`. |
...
| 5.8 | **iCal feeds and announcements** (next batch, server side of 4.6) | S | People live in their own calendars, and a per-instrument or per-person feed URL removes the "is it free?" question. Read-only feeds with a per-user token, generated from the same bookings table, and a one-off .ics download for a range. Same event layout as the local 4.6 (stable `UID` per booking, `STATUS:CANCELLED` for cancelled ones) so a calendar that imported the local file keeps working after a facility migrates; the server writes real timezone-qualified times using the facility timezone from 5.4. A feed only shows what the reader may see under 5.3, never costs. Announcements are a facility-scoped notice shown at login and included in the digest. |
...
| 5.15 | **Instrument maintenance & service log** (next batch, server side of 4.4) | M | The same record as 4.4 — dated entry per instrument with type, vendor, cost, downtime hours, notes, attachments — owned by the facility tenant (5.2) and visible to facility staff only, since vendor costs are facility finance. Adds what one user cannot: entries are written by whichever staff member did the work with actor and time from the audit log (5.9), a downtime entry can be opened from the approval queue when a staff member declines bookings for a broken instrument (5.7), and the scheduler (5.6) reminds the supervising staff (3.6a) when a preventive-maintenance interval or a calibration expiry is due. Feeds 5.14's "equipment state" slice per facility. Imported from the local 4.4 table by 5.12. |
| 5.16 | **Per-lab billing statements and period close** (next batch, server side of 4.5) | M | Same statement as 4.5 — lab, range, every booking and service entry under the Project Costs rule, one total — but issued by the facility, delivered to the PI and the lab members the PI designates (5.3) through the job runner (5.6), and reconciled by them in place: a PI can dispute a row, which opens a thread on that booking for facility staff. **Period close** locks the included rows: cost fields and cancellation decisions on an issued row can only change through a correction entry that appears on the next statement, and the close itself is stamped through 5.9 so finance can prove what was billed and when. 5.10's SAP file is generated from closed statements, never from live rows, so what the lab saw and what finance loads are one document. Statement rows imported from 4.5 arrive already marked issued. |
...
more than one facility's data to be worth building. 5.15 fits the single-facility pilot; 5.16
belongs to the finance phase alongside 5.10, and the extended 5.8 to the notifications phase.
