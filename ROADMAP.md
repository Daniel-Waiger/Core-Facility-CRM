...
Derived from a feature comparison with commercial facility-management systems (first made in
September 2026 and revised as the app moved on). This roadmap keeps **only what a 100%
client-side, single-user app can deliver** — no server, no accounts, no email sending, no
finance-system integration. Anything that needs several people logged in at once, or money to
move, is out of scope for this app and is not planned here.
...
The clearest scheduling gap vs. commercial systems that is purely client-side.
...
| 4.3 | **Training/qualification records** — person × instrument with trainer, date, expiry, and optional prerequisite training on another instrument. No auth exists locally, so this is **staff-facing signals, not enforcement** — which matches the early-adopter reality that core staff operate the app: booking-form warnings ("X's training on Y expired"), plus a **"training status unknown" flag** when a person with no training record on an instrument is added to a booking — covering the "did another staff member already train them?" gap, since all staff records live in the same facility database. | M | Feeds 3.2 (trained-user pool trend per instrument) and 3.4. |
| 4.4 | **Grant budget and remaining balance** — an amount and period on a grant; remaining = amount − charges counted by the Project Costs rule (a cancelled charge counts only if retained). Shown on the grant, on Project Costs and in the booking cost panel as the booking is priced. **Display only**: staff see the balance and decide; a single-user app has no budget holder to ask, so it never blocks a save and never records an exception. | S | Additive columns on `grants`; the balance reads the same aggregation the Project Costs card uses, so the two cannot disagree. Feeds the facility-wide XLSX and the Grants sheet. |

Surveyed in commercial systems and **not planned** until a facility asks for them: tiered
cancellation rules (hours-before-start → percentage charged), peak/off-peak rates by instrument
working hours, room occupancy limits across instruments, period subscriptions, and per-sample
request lines. Each is client-side feasible; none has produced friction in daily use yet, and
Tier 4's rule is that real friction shapes it.

## Milestone — Migration/import tool (important, not now)

Importers for data exported from commercial facility systems — service data dumps and per-page
spreadsheet exports — mapping users, instruments, bookings, grants/budget numbers into the
tracker's schema. Client-side file parsing is feasible with the already-bundled SheetJS
(XLSX/CSV); no server needed.
