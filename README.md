...
- **Multi-Format Report Export:**
  - **XLSX:** Comprehensive multi-sheet workbook (Overview, Milestones, Team, Instruments, Meetings, Service Entries, Files, Research Outputs) via SheetJS; the Reports screen has its own workbook — a **Notes** sheet first, carrying each table's caveats, then utilization, staff time, staff × instrument, projects & groups, stewardship, consults, service entries, breadth, activity mix and funnel, plus a Per-Lab Consults sheet when that opt-in toggle is on.
